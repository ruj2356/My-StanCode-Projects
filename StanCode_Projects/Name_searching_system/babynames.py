...
def search_names(name_data, target):
    ...
    for name in name_data:
        name_check = ''
        for ch in name:
            # Change the names we want to check into upper
            name_check += ch
            name_check = name_check.upper()
        if target.upper() in name_check:
            # Add the first word in name_check into final_word, then change the rest of name_check into
            # lower then add them to final_word
            final_name = ''
            final_name += name_check[0]
            final_name += name_check[1:].lower()
            match_lst.append(final_name)
    return match_lst
