...
def get_average(pixels):
    ...
    for i in range(len(pixels)):
        ...
        g += pixels[i].green
        b += pixels[i].blue
    avg_lst = [r // len(pixels), g // len(pixels), b // len(pixels)]
    return avg_lst
...
def get_best_pixel(pixels):
    ...
    ans = []
    for i in range(len(pixels)):
        if limit > get_pixel_dist(pixels[i], avg_rgb_lst[0], avg_rgb_lst[1], avg_rgb_lst[2]):
            ans = pixels[i]
            limit = get_pixel_dist(pixels[i], avg_rgb_lst[0], avg_rgb_lst[1], avg_rgb_lst[2])
    return ans
